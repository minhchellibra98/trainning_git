...
//        return view('frontend.details',['item'=>$product,'comments'=>$comments]);
        $data['comments']= Comment::where('com_product',$id)->get();
        return view('frontend.details',['item'=>$product],$data);
    }

    public function getCategory($id){
        $data['cateName'] =Category::find($id);
        $data['items'] =Product::where('prod_cate',$id)->orderBy('prod_id','desc')->paginate(5);
    ...
    public function postComment(Request $request,$id){
        ...
        $comment->com_content = $request->xcontent;
